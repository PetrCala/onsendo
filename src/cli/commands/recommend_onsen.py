...
def recommend_onsen(args: argparse.Namespace) -> None:
    ...
    with get_db(url=CONST.DATABASE_URL) as db:
        ...
        for i, (onsen, distance, metadata) in enumerate(recommendations, 1):
            print(f"{i}. {onsen.name} (ID: {onsen.id})")
...
def recommend_onsen_interactive() -> None:
    ...
    with get_db(url=CONST.DATABASE_URL) as db:
        ...
        if not locations:
            print("No locations found. Please add a location first using 'add-location'.")
            return
        ...
        # Get target time
        time_input = input("Target time (YYYY-MM-DD HH:MM, or press Enter for now): ").strip()
        target_time = None
        if time_input:
            try:
        ...
        # Get filters
        exclude_closed = input("Exclude closed onsens? (y/n, default: y): ").strip().lower() != 'n'
        exclude_visited = input("Exclude visited onsens? (y/n, default: n): ").strip().lower() == 'y'
        
        # Get limit
        limit_input = input("Maximum number of recommendations (press Enter for no limit): ").strip()
        limit = None
        if limit_input:
            try:
