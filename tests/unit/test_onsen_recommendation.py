...
class TestOnsenRecommendationEngine:
    ...
    def test_get_unvisited_onsens(self):
        ...
        mock_query = Mock()
        mock_query.distinct.return_value.all.return_value = [(1,), (3,)]  # IDs 1 and 3 visited
        mock_session.query.return_value = mock_query
    ...
    def test_get_location_by_name_or_id_by_name(self):
        ...
        # First query (by ID) should fail
        mock_query1 = Mock()
        mock_query1.filter.return_value.first.return_value = None
        
        # Second query (by name) should succeed
        mock_query2 = Mock()
        mock_query2.filter.return_value.first.return_value = location
...
class TestRecommendOnsens:
    ...
    def test_recommend_onsens_with_limit(self):
        ...
        with patch('src.lib.recommendation.filter_onsens_by_distance') as mock_filter:
            mock_filter.return_value = [(onsen, i * 0.5) for i, onsen in enumerate(onsens)]
    ...
    def test_recommend_onsens_exclude_closed(self):
        ...
        with patch.object(engine, 'get_available_onsens') as mock_available:
            mock_available.return_value = [onsens[0], onsens[2]]  # Only first and third available
    ...
    def test_recommend_onsens_exclude_visited(self):
        ...
        with patch.object(engine, 'get_unvisited_onsens') as mock_unvisited:
            mock_unvisited.return_value = [onsens[0], onsens[2]]  # Only first and third unvisited
